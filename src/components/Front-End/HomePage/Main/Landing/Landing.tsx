...
const Landing = () => {
  ...
  return (
    <div
      className={` grid lg:grid-cols-2 sm:grid-col-1 gap-4  px-10 relative items-center h-[calc(100vh-2rem] ${
        mode === "light" ? "bg-slate-200" : "bg-gray-900"
      }`}
    >
      <motion.div className="rounded-lg relative   pt-20">
        ...
        <div className="flex mb-5 mt-5">
          ...
          <Link
            to={"/register"}
            className=" mb-5 mx-2 px-5  align-middle select-none  text-center uppercase transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none  py-3  rounded-lg bg-DarkB text-white text-x shadow-md shadow-gray-900/10 hover:shadow-lg hover:shadow-gray-900/20 focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none"
          >
      ...
      >
        <img
          src={img}
          className="  rounded-[5rem] rounded-br-[20rem] h-[80%] w-[80%] bg-blue-400 opacity-[0.8]"
        />
      </motion.div>
    </div>
  );
